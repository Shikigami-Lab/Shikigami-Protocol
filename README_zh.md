...
界面内置多套主题、可定制侧边栏，图形化导向开箱即用——好看和好用都不将就。
...
完整人格文件：`profiles/example_luna.json` / `profiles/example_luna_en.json`，`profiles/example_mochi.json` / `profiles/example_mochi_en.json`。
...
- **时空共鸣**：
  ...
  - **感知环境**：了解窗外的雨雪、季节与农历节气。
  - **记得你们的日子**：周年纪念、生日、自定义特殊日期——它会记得，并在合适的时候自然带入对话。
...
- **情绪化 TTS**：四套引擎——**Edge TTS**（免安装，需联网）、**KokoroTTS**（本地 ONNX，约 200ms 延迟）、**GPT-SoVITS**（克隆任意音色）、**Qwen3-TTS**（表现力强，需 GPU）。语气指令与音色随当前情绪状态自动调整。
- **语音输入 (STT)**：**SenseVoice**（快速、多语言）或 **Whisper / faster-whisper**（高精度）。模型下载后完全离线运行。

### 🌱 人格演化（Persona Evolution）

一个月后再来的角色，和第一天认识的那个已经不完全一样——这是设计，不是 Bug。

- **记忆驱动重构**：每隔 N 轮对话，AI 根据积累的事实与好感度阶段，对 `base_prompt` + `style_constraint` 做微小重写。无需手动编辑。
...
- **世界书（Lorebook）**：关键词触发的条目，在命中时将世界设定、角色背景或剧情规则注入 prompt。每条目支持 `keyword`（按关键词触发）、`constant`（始终注入）、`inherit`（继承全局设置）三种模式。
- **AI 补全人设**：根据 `base_prompt` 一键补全情绪描述、自省配置、记忆配置、核心锚点等空白字段。
- **群聊**：多人格同会话，各角色流式输出，发言归属清晰。
- **命令系统**：`/fact`、`/recall`、`/memory`、`/search`、`/todo`、`/timer`、`/help`，以及自然语言触发器（通过关键词匹配自动激活指定 prompt 段落）。
